...
function normalizeVerifyOptions(providedOptions, config) {
    ...
    let linterOptions = config.linterOptions || config;
...
class Linter {
    ...
    verify(textOrSourceCode, config, filenameOrOptions) {
        ...
        if (config) {
            if (config instanceof FlatConfigArray) {
                return this._verifyWithFlatConfigArray(textOrSourceCode, config, options);
            }
    ...
    _verifyWithFlatConfigArrayAndWithoutProcessors(textOrSourceCode, providedConfig, providedOptions) {
        ...
        const configuredGlobals = languageOptions.globals || {};
        const settings = config.settings || {};
        ...
        if (!slots.lastSourceCode) {
            ...
            if (!slots.lastSourceCode.scopeManager) {
                slots.lastSourceCode = new SourceCode({
                    ...
                    parserServices: slots.lastSourceCode.parserServices,
                    visitorKeys: slots.lastSourceCode.visitorKeys,
                    scopeManager: analyzeScope(slots.lastSourceCode.ast, languageOptions.parserOptions)
                });
            }
        }
        ...
        const commentDirectives = options.allowInlineConfig
            ? getDirectiveComments(
                options.filename,
                sourceCode.ast,
                ruleId => FlatConfigArray.getRuleFromConfig(ruleId, config),
    ...
    _verifyWithFlatConfigArray(textOrSourceCode, configArray, options) {
        ...
        // Store the config array in order to get plugin envs and rules later.
        internalSlotsMap.get(this).lastConfigArray = configArray;
        const config = configArray.getConfig(options.filename);

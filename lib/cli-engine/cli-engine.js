...
/** @typedef {import("../shared/types").ErrorInfo} ErrorInfo */
/** @typedef {import("../shared/types").ErrorMap} ErrorMap */
 ...
 * @property {ErrorInfo[]} unaccountedErrors Any errors in the allowlist which were unaccounted for.
 ...
 * @property {ErrorMap} allowlist The allowlist of errors to skip.
 ...
 * @param {ErrorInfo[]|undefined} config.allowedErrors The allowlist for skipping errors.
...
function verifyText({
    ...
    const allowedErrorsMap = {};
    for (const error of (allowedErrors || [])) {
      allowedErrorsMap[transformErrorToKey(error)] = error;
    }
    ...
    messages = messages.filter((message) => {
        const errKey = transformErrorToKey({
            text: message
        });
        if (allowedErrorsMap[errKey]) {
            // Only allow it to be used once.
            allowedErrorsMap[errKey] = false;
            return false;
        }
        return true;
    });
    ...
    const result = {
        filePath,
        messages,
        unaccountedErrors: allowedErrorsMap.values(),
...
/**
 * Transforms an ErrorInfo object to a string key.
 * @param {ErrorInfo} error
 * @returns {string}
 * @private
 */
function transformErrorToKey(error) {
    return `${x.line || 'UNKNOWN'}::${x.column || 'UNKNOWN'}::${x.ruleId || 'UNKNOWN'}`;
}
...
/**
 * Loads and returns the requested allowlist.
 * @param {string|null} allowlistPath The filepath for the allowlist, if provided.
 * @returns {ErrorMap} A loaded allowlist (or empty if not given a path)
 */
    ...
    executeOnFiles(patterns) {
        ...
        for (const { config, filePath, ignored } of fileEnumerator.iterateFiles(patterns)) {
            ...
            const result = verifyText({
                ...
                fileEnumerator,
                linter,
                allowedErrors: allowlist[filePath]
            });

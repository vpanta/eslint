...
const FlatConfigArraySymbol = {
    pluginRules: Symbol("rules")
};
...
function populatePluginRulesMap(configs) {
    const pluginRules = configs[FlatConfigArraySymbol.pluginRules];

    for (const config of configs) {
        if (config.plugins) {
            for (const [pluginName, plugin] of Object.entries(config.plugins)) {

                // core rules are tracked elsewhere
                if (pluginName === "@") {
                    continue;
                }
                ...
                if (plugin.rules) {
                    for (const ruleName of Object.keys(plugin.rules)) {
                        pluginRules.set(`${pluginName}/${ruleName}`);
                    }
                }
            }
        }
...
class FlatConfigArray extends ConfigArray {
    ...
    constructor(configs, { basePath, baseConfig = defaultConfig }) {
        ...
        /**
         * Map of plugin rules in plugin/rule-name format.
         * @type {Map<string,import("../shared/types").Rule>}
         * @private
         */
        this[FlatConfigArraySymbol.pluginRules] = new Map();
    }
    ...
    async normalize(context) {
        ...
        populatePluginRulesMap(this);

        return super.normalize(context);
    }

    async normalizeSync(context) {
